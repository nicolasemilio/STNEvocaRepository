...
plotNet <-function(N, tit, nsizef, ewidthf, asize, ecurv, mylay) 
{
  ...
  plot(N, layout = mylay, vertex.label = "", vertex.size = nsize, main = title,
       edge.width = ewidth, edge.arrow.size = asize*0.7, edge.curved = ecurv, asp=0)#, axes=TRUE) #,xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5)) 
...
stn_plot <- function(inst)  {
  ...
  #lkk2 <- lkk
  #lkk2 <- as.matrix(lkk2)
  #lkk2[, 2] <- lkk2[, 2] - (V(STN)$fitness)  
  
  # Aplicar separación
  lkk <- separar_nodos(lkk, umbral = 0.5, desplazamiento = 1.2)
  #lkk2 <- separar_nodos(lkk, umbral = 0.8, desplazamiento = 1.4)
  lfr <- separar_nodos(lfr, umbral = 0.05, desplazamiento = 0.1)
  #lkk2 <- separar_nodos_dinamico(STN, lkk, base_umbral = 0.3, desplazamiento = 0.4, max_iter = 1)

  
  plotNet(N = STN, tit="FR Layout.", nsizef=size_factor*0.3, ewidthf=size_factor*0.6, asize=0.3, ecurv=0.3, mylay=lfr)
  plotNet(N = STN, tit="KK Layout.", nsizef=size_factor*0.3, ewidthf=size_factor*0.6, asize=0.2, ecurv=0.3, mylay=lkk)
  #plotNet(N = STN, tit="KK2 Layout.", nsizef=size_factor*0.3, ewidthf=size_factor*0.6, asize=0.2, ecurv=0.3, mylay=lkk2)
