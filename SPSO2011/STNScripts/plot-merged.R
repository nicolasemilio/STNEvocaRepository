...
stn_decorate <- function(N)  {
  ...
  V(N)[V(N)$color == shared_col]$frame.color <- "pink"
...
#lzfr   <-layout_with_fr(zoom)
#lzkk <-layout_with_kk(zoom)

ofname <-  gsub('.{6}$', '', infile) # removes  (last 6characters) .RData from file to use as name
ofname = paste0(ofname,'-plot.pdf')

separar_nodos <- function(layout, umbral, desplazamiento) {
  for (i in 1:(nrow(layout) - 1)) {
    for (j in (i + 1):nrow(layout)) {
      dist_ij <- sqrt(sum((layout[i, ] - layout[j, ])^2))
      if (dist_ij < umbral) {
        # Mover ambos nodos aleatoriamente en direcciones distintas
        mov <- runif(2, -desplazamiento, desplazamiento)
        layout[i, ] <- layout[i, ] + mov
        layout[j, ] <- layout[j, ] - mov
      }
    }
  }
  return(layout)
...
comp <- components(stnm)

# Layout final inicializado
layout_final <- matrix(NA, nrow = vcount(stnm), ncol = 2)
# Procesar cada componente
offset_x <- 0
for (c_id in seq_len(comp$no)) {
  sub_nodes <- which(comp$membership == c_id)
  sg <- induced_subgraph(stnm, sub_nodes)
  
  # Layout para subgrafo
  l_sub <- layout_with_kk(sg)
  
  # Separar nodos solo dentro de la componente
  l_sub <- separar_nodos(l_sub, umbral = 0.5, desplazamiento = 0.5)
  
  # Aplicar desplazamiento global para que no se monten
  l_sub[,1] <- l_sub[,1] + offset_x
  
  # Guardar en layout final
  layout_final[sub_nodes, ] <- l_sub
  
  # Actualizar desplazamiento en X
  offset_x <- offset_x + max(l_sub[,1]) - min(l_sub[,1]) + 3
}


# Aplicar separación
 lkk2 <- separar_nodos(layout_final, umbral = 1, desplazamiento = 1)
 lkk3 <- separar_nodos(layout_final, umbral = 2, desplazamiento = 2)
 lkk4 <- separar_nodos(layout_final, umbral = 3, desplazamiento = 3)
 lkk5 <- separar_nodos(layout_final, umbral = 4, desplazamiento = 4)

#print(lkk)
#lkk2 <- lkk^2
#lkk3 <- separar_nodos(lkk2, umbral = 1, desplazamiento = 5)
...
plotNet(stnm, nsizef=nf, ewidthf=ef*0.5, asize=0.12, ecurv=0.3, mylay= layout_final,bleg = T, tit=" ")
# Slightly smaller nodes and edges for the KK layout as it spreads the components and makes the nodes
# closer to each other
plotNet(stnm, nsizef=nf*0.9, ewidthf=ef*0.5, asize=0.12, ecurv=0.3, mylay= lkk2,bleg = F, tit=" ") #,tit="KK Layout")
plotNet(stnm, nsizef=nf*0.9, ewidthf=ef*0.5, asize=0.12, ecurv=0.3, mylay= lkk3,bleg = F, tit=" ")#, tit="LN layout")
# Slightly smaller nodes and edges for the KK layout as it spreads the components and makes the nodes
# closer to each other
plotNet(stnm, nsizef=nf*0.9, ewidthf=ef*0.5, asize=0.12, ecurv=0.3, mylay= lkk4,bleg = F, tit=" ")#,tit="LE Layout")
plotNet(stnm, nsizef=nf*0.9, ewidthf=ef*0.5, asize=0.12, ecurv=0.3, mylay= lkk5,bleg = F, tit=" ") #tit="LE Layout")

# Plots of zoomed  -- Increased summed size and edges as the zoomed network has less nodes 
#nf <- size_factor*1.5  
#ef <- size_factor

#plotNet(N = zoom, tit="Zoomed (top 25%) FR", nsizef=nf*0.1, ewidthf=ef*0.3, asize=0.35, ec urv=0.3, mylay=lfr, bleg = T)
#plotNet(N = zoom, tit="Zoomed (top 25%) KK", nsizef=nf*0.1, ewidthf=ef*0.3, asize=0.2, ecurv=0.3, mylay=lkk, bleg = T)
